"use client"

import { useAuth } from "@/hooks/use-auth"
import { useFirestoreSync } from "@/hooks/use-firestore-sync"
import { Button } from "@/components/ui/button"
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
  DropdownMenuSeparator,
  DropdownMenuTrigger,
} from "@/components/ui/dropdown-menu"
import { cn } from "@/lib/utils"
import { Loader2, CheckCircle, WifiOff } from "lucide-react"

function GoogleLogo({ className }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 24 24" aria-hidden>
      <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
      <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
      <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l3.66-2.84z"/>
      <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
    </svg>
  )
}

function SyncIndicator({ isSyncing, syncError }: { isSyncing: boolean; syncError: Error | null }) {
  if (syncError) return <WifiOff className="w-3 h-3 text-amber-500" />
  if (isSyncing) return <Loader2 className="w-3 h-3 animate-spin text-muted-foreground" />
  return <CheckCircle className="w-3 h-3 text-green-500" />
}

export function UserButton() {
  const { user, isAuthLoading, signIn, signOut } = useAuth()
  const { isSyncing, syncError } = useFirestoreSync()

  if (isAuthLoading) {
    return <div className="h-9 w-36 rounded-full bg-muted animate-pulse" />
  }

  if (!user) {
    return (
      <Button
        variant="outline"
        size="sm"
        className="gap-2 rounded-full"
        onClick={signIn}
      >
        <GoogleLogo className="w-4 h-4" />
        Sign in with Google
      </Button>
    )
  }

  const firstName = user.displayName?.split(" ")[0] ?? "User"

  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <button className={cn(
          "flex items-center gap-2 rounded-full border border-border bg-card px-3 py-1.5",
          "text-sm font-medium hover:bg-muted/60 transition-colors"
        )}>
          {user.photoURL && (
            <img
              src={user.photoURL}
              alt={user.displayName ?? ""}
              className="w-6 h-6 rounded-full ring-1 ring-border"
            />
          )}
          <span className="max-w-[96px] truncate">{firstName}</span>
          <SyncIndicator isSyncing={isSyncing} syncError={syncError} />
        </button>
      </DropdownMenuTrigger>
      <DropdownMenuContent align="end" className="w-52">
        <div className="px-3 py-2 text-xs text-muted-foreground truncate">{user.email}</div>
        <DropdownMenuSeparator />
        <div className="px-3 py-1.5 text-xs text-muted-foreground flex items-center gap-1.5">
          <SyncIndicator isSyncing={isSyncing} syncError={syncError} />
          {syncError ? "Offline — saved locally" : isSyncing ? "Syncing…" : "Progress synced"}
        </div>
        <DropdownMenuSeparator />
        <DropdownMenuItem
          className="cursor-pointer text-destructive focus:text-destructive"
          onSelect={signOut}
        >
          Sign out
        </DropdownMenuItem>
      </DropdownMenuContent>
    </DropdownMenu>
  )
}
